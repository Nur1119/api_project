...
## Текущий статус
**Фаза 1 — в процессе.**
- Сделано: проект инициализирован, трекер заведён, **JDK 25 (Temurin LTS)** работает (`java -version` = 25.0.3), IntelliJ есть, **Git + GitHub синхронизированы** (`origin` = Nur1119/api_project, первый коммит запушен), **ключ Nessie получен и лежит в `.env`** (gitignored), проверен живым запросом `GET /accounts` → `200`.
- **Сейчас:** шаг 5 — каркас Maven-проекта (`pom.xml`, структура `src/test/java`, Maven wrapper).
- Дальше: первый smoke-тест (`GET /accounts?key=...` → `200`).
