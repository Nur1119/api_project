...
## Текущий статус
**Фаза 1 — в процессе.**
- Сделано: проект инициализирован, трекер заведён, **JDK 25 (Temurin LTS)** работает (`java -version` = 25.0.3), IntelliJ есть, **Git + GitHub синхронизированы** (`origin` = Nur1119/api_project, первый коммит запушен).
- **Сейчас:** шаг 4 — получить ключ Nessie и положить в env (не в репо).
- Дальше: каркас Maven → smoke-тест.
